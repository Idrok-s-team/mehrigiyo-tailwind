...
const DoctorCard: FC<Props> = ({ data, className }) => {
  ...
  const handleAppointment = () => {
    if (isUserRegistered) {
      ...
      toast.error(`Shifokor bilan uchrashuv belgilash uchun ${WARNING_TEXTS.PLEASE_REGISTER_FIRST}`)
    }
  }

  const cardClasses = clsx(
    'w-full h-[250px] bg-white border border-[#E2E2E2] shadow-secondary rounded-[18px] p-4',
    className,
  )

  return (
    <article key={id} className={cardClasses} title={full_name}>
